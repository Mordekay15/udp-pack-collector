...
void *count_statistics()
{
	...
	if (size != cvector_size(&vec)) {
	    pthread_mutex_lock(&mtx);

	    for(int i = 0; i < cvector_size(&vec); i++)
		bytes += cvector_get(&vec, i);

	    char* p_arr = convert_statictics(cvector_size(&vec));
	    char* b_arr = convert_statictics(bytes);
